...
def varmint2(T):
  lut = defaultdict(lambda: defaultdict(list))
  for vx,v in enumerate(T):
    vl = [i for i,e in enumerate(v) if e == 0]
    for ux in range(vx):
      u = T[ux]
      ul = [i for i,e in enumerate(u) if e == 0]
      if vl == ul:
        lut[ux][0].append(vx)
        lut[vx][0].append(ux)
  return lut
        ...
        while True:
          ...
          if score < lastscore:
            ...
            if lastscore < a_score:
              a_score = lastscore
              ANODE = deepcopy(A)
              a_problems = init_problems(ANODE,d,a_foes)
              a_score = sum(map(len,a_problems))
              print(datetime.now(), '::', a_score,u,x, len(ANODE))
...
a_problems = init_problems(A,d,a_foes)
a_score = sum(map(len,a_problems))
print('Sanity:', a_score, len(A))

print('Dumping', w)
dump_pa(A, f'pa_nest_{n}_{d}.txt')

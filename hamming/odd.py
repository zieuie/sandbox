...
"""
Reconstructed Python 3 version of the original `odd.py`.

The historical dependencies (`xutils.xfield`, `xutils.xflow`, `xtar`, etc.)
are not present in this workspace, so this file restores the original control
flow with local replacements:
...
- simple helpers for parsing prime powers and serializing output.

The underlying construction follows the surviving code/comments closely. The
missing `gamma`/`gammainverse` helpers are reconstructed as cyclic translation
 modulo q, which is consistent with how this file uses them.
"""
...
It uses a parity argument to create a good coverage of AGL.
...
-w, --write      Write output to file without prompting
...
def cyclic_shift_inverse(value: int, shift: int, modulus: int) -> int:
...
def replicate(script_p: Sequence[Iterable[int]], k: int, modulus: int) -> list[list[int]]:
    return [[cyclic_shift_inverse(p, k, modulus) for p in p_set] for p_set in script_p]


def make_pis(n: int, r: int, verbose: bool = False, naive: bool = False, very_verbose: bool = False) -> tuple[list[list[list[int]]], list[list[list[int]]], GaloisField, int]:
    nr = n**r
    if verbose:
        print(f"Computing a partition for {n}^{r} = {nr}")

    if naive:
        if verbose:
            print("Exercising naivite...")
        pi_p, pi_q, field, value = naive_theorem(n, r, verbose=very_verbose)
    else:
        if verbose:
            print("Running optsud...")
        pi_p, pi_q, field, value = theorem(n, r, verbose=very_verbose)
    ...
    p_level_1 = []
    q_level_1 = []
    for s in range(num_pis):
        p_level_1.append(replicate(pi_p, s * (k + 1), nr))
        q_level_1.append([list(q) for q in pi_q])

    total = (value + nr) * num_pis
    if verbose:
        percent = 100.0 * total / float(nr * (nr - 1))
        print(f"Generated {num_pis} pis, each with {value + nr} permutations, totalling {percent:.2f}% of AGL")

    return p_level_1, q_level_1, field, total


def serialize_field(field: GaloisField) -> dict[str, object]:
    return {
        "prime": field.P,
        "degree": field.R,
        "order": field.Q,
...
def write_output(filename: str, payload: dict[str, object]) -> None:
    path = Path(filename)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-r", "--replicate", action="store_true", help="Create pis")
    ...
    parser.add_argument("-w", "--write", action="store_true", help="Write output without prompting")
...
def should_write(force: bool) -> bool:
    if force:
        return True
    answer = input("Write to file? (Y/n): ").strip().lower()
    return answer != "n"
    ...
    if args.replicate:
        if args.peek:
            if args.naive:
                ...
                value, split = dp_split(n, r)
                k = sum(step[2] for step in split)
                num_pis = (nr - 1) // (k + 1)
                if args.verbose:
                    print(split)

            total = num_pis * (value + nr)
            percent = 100.0 * total / float(nr * (nr - 1))
            print(f"Can create {num_pis} pis, each with {k + 1} cosets (probably)")
            print(f"With each pi having {value + nr} permutations, that is {total} permutations total")
            print(f"This is {percent:.2f}% of AGL")
        else:
            p_level_1, q_level_1, field, coverage = make_pis(
                n,
                r,
                verbose=True,
                naive=args.naive,
            ...
            print(f"Created {len(p_level_1)} pis, each with {len(p_level_1[0])} cosets")
            print(f"That is {coverage} permutations total")
            print(f"This is {100.0 * coverage / float(nr * (nr - 1)):.2f}% of AGL")

            if should_write(args.write):
                filename = f"Pis_{nr + 1}_{len(p_level_1)}_{coverage}_{'naive' if args.naive else 'dp'}.xtar.json"
                write_output(
                    filename,
                    {
                        "filetype": "pi",
        ...
        if args.peek:
            ...
            if should_write(args.write):
                filename = f"M_{nr + 1}_{nr}_{coverage}.xtar.json"
                write_output(
                    filename,
                    {
                        "filetype": "pe",
                        "pa": {"type": "agl", "field": serialize_field(field)},
                        "P": [sorted(list(p)) for p in p_sets],
                        "Q": q_sets,
                    },
                )
                print("Success!")

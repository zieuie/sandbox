...
def find_addable(X, Y, x, y, root_color):
  frontier = list(Y[-1].keys()) if len(Y) > 1 else [root_color]
  if not frontier:
    VERBOSE and print("find_addable: EMPTY frontier")
    return None

  # pick one class to diagnose deeply
  a0 = frontier[0]
  counts = Counter()
  seen = 0
  for v in it.islice(from_color(a0), 200):
    seen += 1
    r = why_not_addable(X, Y, x, y, a0, v)
    counts[r] += 1
    if r is None:
      VERBOSE and print("FOUND addable in diagnostic scan for", a0)
      return a0, v

  VERBOSE and print("Diagnostic for class", a0, "scanned", seen, "candidates")
  VERBOSE and print("Reject reasons:", counts)

  # if none found in first 200, continue normal search (or just bail for debugging)
  for a in frontier:
...
def find_it(M, colors):
  while len(M) < len(colors):
    for A in colors:
      if A not in M:
        break
    else:
      raise ValueError('Somehow all colors were in M, but M was smaller than colors.')
...
def why_not_addable(X, Y, x, y, a, v):
  if a in Y[-1] and v == Y[-1][a]:
    return "equals_Yl"
  if a in x and v in x[a]:
    return "already_in_x"
  ...
  for us in x.values():
    for u in us:
      if edge(u, v):
        return "hits_x"
  for layer in Y:
    for u in layer.values():
      if edge(u, v):
        return "hits_Y_le"
  for layer in X:
    for us in layer.values():
      for u in us:
        if edge(u, v):
          return "hits_X_le"
  return None

...
def load_pa_2(n, d, must_be_done=False):
  unf_file = f'pa_{n}_choose_{d}_symmetric_unfinished.txt'
  fin_file = f'pa_{n}_choose_{d}_verified.txt'

  if must_be_done:
    if os.path.exists(fin_file):
      return load_pa(fin_file)
    else:
      print(f'Cannot find a finished PA for {n}, {d}')
      exit(1)

  if n//d == 2:
    print(f'Randomly generating seed PA {n-d}, {d}')
    A = [list(range(n-d))]
    return enweave(A, n, d)

  elif os.path.exists(unf_file):
    print(f'Loading from {unf_file}')
    return load_pa(unf_file)

  elif n-d > d:
    print(f'Recursing to {n-d}, {d}')
    A = load_pa_2(n-d, d, must_be_done=True)
    return enweave(A, n, d)

  else:
    print('Cannot generate seed PA')
    exit(1)
...
if __name__ == '__main__':
  ...
  if n%d != 0 or n <= d:
    print ('This program only works when d | n.')
    exit(1)

...
def main(n,d):
  ...
  for it_count in it.count():
    ...
    for _, ux in problems:
      ...
      # print('Making pins')
      start = time()
      pins = make_possibilities(n,d,u, [A[vx] for vx in umad])
      print('Made pins', len(pins), time()-start, ux, umad)
      for upot in pins:
      ...
      print(f'Pins: {len(pins)} Pots: {len(pots)} Best: {pots[0][0] if pots else None}')

      # okay just take the best
      if pots:
        _, upot, gain, loss
        A[ux] = upot
        for vx in gain:

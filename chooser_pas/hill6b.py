
from copy import deepcopy
import random
import itertools as it
from datetime import datetime
from lib import *


def main(n, k, d):
  # Step 1 - Make a dumb PA
  A, H, L = dumb_pa(n, k)
  try:
    A = load_pa(filename)
  except FileNotFoundError:
    pass

  s = init_separations(A, d)
  W = len(A) * (len(A)-1)

  best_score = float('inf')
  best_pa = deepcopy(A)
  count_disturbs = 0

  try:
    for it_count in it.count():
      w = sum(len(e) for e in s)
      coverage = sum(1 for e in s if len(e) == len(A)-1)
      uncovered = list(i for i,e in enumerate(s) if len(e) != len(A)-1)

      # should_print = it_count % 100 == 0
      should_print = True
      if W-w < best_score:
        should_print = True
        best_score = W-w
        best_pa = deepcopy(A)
        if w == W:
          return A

      should_print and print(datetime.now(), 'Iteration:', it_count, 'Score:', W-w, 'Best:', best_score, 'Coverage:', coverage, 'of', len(A), 'Disturbs:', count_disturbs)

      # for i in random.choices(list(range(len(A))), k=100):
      random.shuffle(uncovered)
      for i in uncovered:
        if random_good_permutation(A, H, L, s, i, d, loops=1000):
          break
      else:
        gently_disturb(A, H, L, s, d)
        # greatly_disturb(A, H, L, s, d)
        count_disturbs += 1

  except KeyboardInterrupt:
    return best_pa


if __name__ == '__main__':
  from sys import argv
  n, d = int(argv[1]), int(argv[2])

  # The original PA is size m
  # filename = f'pa_{n}_choose_{d}.txt'
  filename = f'dump88.txt'
  pa = main(n, d, d)
  with open(filename, 'w+') as f:
    for row in pa:
      f.write(' '.join(map(str, row)) + '\n')

